...
impl UploadBufferOp {
    ...
    pub fn resource_state(&self, bundle: &RendererBundle) -> Option<Vec<ResourceState>> {
        bundle.buffer_container.get_general_buffer(self.buff)?;
        let res = ResourceId::Buffer(self.buff);
        Some(
            [ResourceState::new(
                res,
...
impl UploadImageOp {
    ...
    pub fn resource_state(&self, bundle: &RendererBundle) -> Option<Vec<ResourceState>> {
        bundle.texture_container.get_image(self.texture_id)?;
        let res = ResourceId::Texture(self.texture_id);
        Some(
            [ResourceState::new(
                res,
